...
### Per Feature Linear kernel class 
# ---------------------------
# Custom Atmospheric Kernel (Paper-correct)
# ---------------------------
class PerFeatureLinearKernel(gpytorch.kernels.Kernel):
    """
    Atmospheric kernel:
    k(x1, x2) = Σ_q w_q^2 * (u1[q] * u2[q])
    """
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        # ---- FIX: manually extract active dims ----
        if self.active_dims is not None:
            x1 = x1[..., self.active_dims]
            x2 = x2[..., self.active_dims]
        # ------------------------------------------

        # Now x1, x2 are (N, Q) and (M, Q)
        w2 = self.weights ** 2  # shape (Q,)

        if diag:
            # elementwise: sum_q w_q^2 * x1[n,q] * x2[n,q]
            return torch.sum(w2 * x1 * x2, dim=-1)

        # full covariance: (x1*w2) dot (x2^T)
...
class SingleIsoGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, n_coords=2, p_atm=0):
        ...
        self.mean_module = gpytorch.means.LinearMean()  # for OK: constant Mean 
        # PAPER: Matern 1.5 with separate lengthscales for latitude/longitude
        matern = gpytorch.kernels.MaternKernel(
            nu=1.5,
            ard_num_dims=n_coords,
            active_dims=list(range(0, n_coords))
        )
        matern = gpytorch.kernels.ScaleKernel(matern)
        if p_atm > 0:
            atm_active = list(range(n_coords, n_coords + p_atm))
            atm_kernel = PerFeatureLinearKernel(
                num_dimensions=p_atm,
                active_dims=atm_active
            )
...
if __name__ == "__main__":
    ...
    isotopes = ["Li", "Na", "Mg"] # "Al", "Si", "P", "K", "Ca", "Ti", "Cr", "Mn", "Fe", "Co", "Ni", "Cu", "Zn", "Ga", "As", "Rb", "Sr",
   ...
   # ]
    ...
    test_samples_df = test_candidates.iloc[[22, 25, 43, 1]][['Latitude','Longitude'] + isotopes].reset_index(drop=True)
